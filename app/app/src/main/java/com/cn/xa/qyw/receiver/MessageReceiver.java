package com.cn.xa.qyw.receiver;
    ...
    public void onTextMessage(Context context, XGPushTextMessage message) {
        ...
        if (customContent != null && customContent.length() != 0) {
            try {
                ...
                Push push = com.alibaba.fastjson.JSONObject.parseObject(customContent,Push.class);

                String userName = PreferenceUtils.getPrefString(PreferenceKeys.USER_ID, "");

                if(!StringUtils.isEmpty(userName)){
                    push.setToUserId(userName);

                    if(push.getUserId().equals(push.getToUserId())){
                        return;
                    }

                    HttpUtils.postDataFromServer(HttpAddress.PUSH_CONTENT, com.alibaba.fastjson.JSONObject.toJSONString(push), new NetworkResponseHandler() {
                        @Override

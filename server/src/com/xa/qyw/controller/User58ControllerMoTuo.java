...
@Controller
@RequestMapping("/api/user58motuo/")
public class User58ControllerMoTuo {
	...
	public String loginUser() {
		...
		try {
			...
			new Thread() {
				...
				public void run() {
					String mainUrl = "http://xa.58.com/danche/";
					try {
						for (int i = 1; i < 12; i++) {
							String url = "";
							if (i == 1) {
	...
	private String getPhoneNumber(Document doc) {
		...
		try {
			...
			Element elsTitle = doc.select("span.f20").first();
			if (elsTitle != null) {

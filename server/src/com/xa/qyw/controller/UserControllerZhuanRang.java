...
@Controller
@RequestMapping("/api/shipinbaojian/")
public class UserControllerZhuanRang {
	...
	public String loginUser() {
		...
		try {
			...
			new Thread() {
				...
				public void run() {
					String mainUrl = "http://xian.liebiao.com/shipinbaojian/";
					try {
						for (int i = 1; i < 21; i++) {
							String url = "";
							if (i == 1) {
							...
							final Elements els = doc.select("a.text-highlight");
	...
	private String getIntro(Document doc) {
		try {
			...
			Element elP = elsTitle.select("p").first();
			return elP.text();
		} catch (Exception e) {
			e.printStackTrace();
		}

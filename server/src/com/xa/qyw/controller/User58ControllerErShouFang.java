...
@Controller
@RequestMapping("/api/user582shou/")
public class User58ControllerErShouFang {
	...
	public String loginUser() {
		...
		try {
			...
			new Thread() {
				...
				public void run() {
					String mainUrl = "http://xa.58.com/yangling/ershoufang/";
					try {
						for (int i = 1; i < 5; i++) {
							String url = "";
							if (i == 1) {
								url = mainUrl
										+ "?PGTID=0d30000c-001e-3e89-b8d5-456119efe5ea&ClickID=5";
							...
							final Elements els = doc.select("p.bthead");
	...
	private void getConnect(String url) {
		try {
			...
			if (!StringUtils.isEmpty(name) && StringUtils.isTelActive(phoneNum)
					&& !phoneNum.contains("*")) {
				...
				if (simUser == null) {
					...
					userInfo.setArea("杨凌县");
					userInfo.setDepartmentId(279);
	...
	private String getUserPhoto(Document doc) {
		try {
			...
			Element elsTitle = doc.select("li.li_jjrimg").first();
	...
	private String getTrueName(Document doc) {
		...
		try {
			Element elsTitle = doc.select("li.li_jjrname").first();

			if (elsTitle != null) {
				Element els = elsTitle.select("a").first();
